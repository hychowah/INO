...
async def create_relation(req: CreateRelationRequest):
    ...
    if req.relation_type not in db.VALID_RELATION_TYPES:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Invalid relation_type. Must be one of: "
                f"{', '.join(sorted(db.VALID_RELATION_TYPES))}"
            ),
        )
    ...
    if relation_id is None:
        raise HTTPException(
            status_code=400,
            detail=(
                "Relation rejected (duplicate, cap exceeded, "
                "self-referential, or invalid concept IDs)"
            ),
        )
