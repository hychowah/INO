...
import db
from webui.helpers import _esc, _parse_action_summary, _relative_time, layout
...
def page_actions(query_string: str = "") -> str:
    ...
    if not entries:
        table_html = (
            '<div class="empty">No actions logged yet. Start learning via the Discord bot!</div>'
        )
    else:
        rows_html = ""
        for e in entries:
