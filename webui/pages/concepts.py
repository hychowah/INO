...
import db
from webui.helpers import _esc, layout, score_bar
...
def page_concept_detail(concept_id: int) -> str:
    ...
    if relations:
        ...
        for rel in relations:
            ...
            note_html = (
                f'<div style="font-size:12px;color:var(--text2);margin-top:2px">{rel["note"]}</div>'
                if rel.get("note")
                else ""
            )
    ...
    if p1_raw:
        try:
            ...
            if p1_cids:
                cid_links = ", ".join(
                    f'<a href="/concept/{_esc(str(cid))}">#{_esc(str(cid))}</a>' for cid in p1_cids
                )
                p1_meta_parts.append(f'<span class="p1-tag">Concepts: {cid_links}</span>')
            p1_meta = (
                f'<div class="p1-meta">{" ".join(p1_meta_parts)}</div>' if p1_meta_parts else ""
            )
            p1_reasoning_html = (
                f'<div class="p1-reasoning">{p1_reasoning}</div>' if p1_reasoning else ""
            )
