...
def render_tree_node(topic, by_id, subtree_stats, depth=0, expanded_default=True):
    ...
    if total_due > 0:
        due_text = (
            f"{total_due} due"
            if total_due == own_due or not has_children
            else f"{own_due}/{total_due} due"
        )
    ...
    if has_children:
        ...
        for cid in topic["child_ids"]:
            child = by_id.get(cid)
            if child:
                html += render_tree_node(
                    child, by_id, subtree_stats, depth + 1, expanded_default=False
                )
        html += "</div>"
...
def page_dashboard() -> str:
    ...
    if topic_map:
        ...
        topic_html = (
            '<div class="card"><p>No topics yet. Start learning via the Discord bot!</p></div>'
        )
    ...
    return layout(
        "Dashboard",
        stats_html + due_html + activity_html + topic_html,
        active="",
