...
async def handle_dedup_check() -> list[dict] | None:
    ...
    prompt = f"""\
You are a dedup agent for a learning system. Your ONLY job is to find TRUE \
duplicates — concepts that are the SAME thing with different wording.
...
2. For each duplicate group, pick the ONE concept to KEEP — prefer the one with more \
reviews, then the most specific/descriptive title.
...
- "Ring Buffer Lock-Free Pattern" and "ISR Bottom Half Pattern" → different techniques, \
even if related
- "Reset Vector & Memory Layout" and "Embedded Bootloader" → specific vs broad, \
...
**When in doubt, do NOT merge.** It is far better to keep two similar concepts separate than to \
merge concepts that cover different learning goals. The user needs granular concepts for \
        ...
        for mid in merge_ids:
            ...
            if detail and detail.get("remarks"):
                for remark in detail["remarks"]:
                    db.add_remark(
                        keep_id,
                        f"[merged from #{mid} '{merge_concept['title']}'] {remark['content']}",
                    )
...
def format_dedup_suggestions(groups: list[dict]) -> str:
    ...
    header = (
        "🔄 **Potential Duplicate Concepts** ({} group(s))\n\n"
